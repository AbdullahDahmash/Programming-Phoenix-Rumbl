defmodule Rumbl.AuthTest do
  ...
  test "login with not found user", %{conn: conn} do
    {:error, :not_found, _conn} = Auth.login_by_username_and_pass(conn, "me", "secret", repo: Repo)
  end

  test "login with a password mistmatch", %{conn: conn} do
    user = insert_user(username: "me", password: "secret")
    {:error, :unauthorized, conn} = Auth.login_by_username_and_pass(conn, "me", "not_secret", repo: Repo)
  end
end
